-- Hopper PRO - VersiÃ³n Final Mejorada
print("ðŸš€ HOPPER PRO - VersiÃ³n Mejorada")
...
local function sendToDiscord(brainrotData)
    ...
    -- Crear URL para unirse (funciona en mÃ³vil y PC)
    ...
    local embed = {
        title = "ðŸŽ¯ LKS Notify | Notify 10m+ plus",
        color = 15158332,
        fields = {
        ...
        footer = {text = "LKS Notify | by Hopper PRO | " .. os.date("%H:%M") .. " (HorÃ¡rio de BrasÃ­lia) | hoy a las " .. os.date("%H:%M")}
    }
    ...
    if success then
        print("âœ… Enviado a Discord con link clickeable")
    end
...
title.Text = "ðŸ¤– HOPPER PRO - Auto Scan"
...
statusText.Text = "âœ… GUI Cargado\nðŸŽ¯ Escaneo automÃ¡tico activado\nðŸ“Š Servidores: 0 | ðŸ’¬ Webhook: âœ…"
...
scanBtn.Text = "ðŸ” ESCANEAR AHORA"
...
hopBtn.Text = "ðŸ”„ SALTAR A OTRO SERVIDOR"
...
local function scanServer()
    print("ðŸ” Escaneando:", game.JobId)
    local found = {}
...
local function updateResultsList()
    ...
    if #goodBrainrots == 0 then
        statusText.Text = string.format("âŒ Sin Brainrots\nðŸ“Š Servidores: %d\nðŸ’¬ Webhook: âœ…", serversChecked)
        return
    end
    ...
    for i, br in ipairs(goodBrainrots) do
        ...
        label.Text = "ðŸ’Ž " .. br.name .. " - " .. formatNumber(br.generation) .. "/s"
    ...
    statusText.Text = string.format("âœ… %d Brainrots!\nðŸ“Š Servidores: %d\nðŸ’¬ Webhook: %d enviados", #goodBrainrots, serversChecked, #goodBrainrots)
end

-- SERVER HOP mejorado (evita servidores repetidos)
local function serverHop()
    print("ðŸ”„ Buscando servidor diferente...")
    statusText.Text = "ðŸ”„ Saltando...\nâ³ Espera..."
...
local function startAutoMode()
    ...
    print("ðŸ¤– Modo auto activado")
    statusText.Text = "ðŸ” Escaneando..."
    ...
    if #found > 0 then
        updateResultsList()
        showNotification(string.format("ðŸŽ‰ %d Brainrot(s)!\nðŸ’¬ Enviado a Discord", #found), Color3.fromRGB(50, 200, 100))
    end
    
    task.wait(3)
    
    if isExploring then
        statusText.Text = "ðŸ”„ Saltando a otro servidor..."
        task.wait(1)
        serverHop()
    end
end

-- âœ¨ ESCANEO AUTOMÃTICO AL ENTRAR âœ¨
print("â³ Esperando carga del servidor...")
task.wait(SCAN_TIME)

print("ðŸ” Escaneando automÃ¡ticamente al entrar")
statusText.Text = "ðŸ” Escaneando automÃ¡ticamente..."
...
if #found > 0 then
    ...
    updateResultsList()
    showNotification(string.format("ðŸŽ‰ %d Brainrot(s) al entrar!\nðŸ’¬ Enviado a Discord", #found), Color3.fromRGB(50, 200, 100))
    print("âœ… Encontrados", #found, "Brainrots al entrar al servidor")
else
    statusText.Text = string.format("âŒ No hay Brainrots aquÃ­\nðŸ“Š Servidor %d escaneado\nðŸ’¬ Webhook: âœ…", serversChecked)
    print("âŒ No se encontraron Brainrots de 10M+ en este servidor")
end

-- Si el modo auto estÃ¡ activo, continuar
if _G.HopperAuto then
    print("ðŸ¤– Modo auto detectado, continuando...")
    if isExploring then
        task.wait(2)
        startAutoMode()
    end
...
webhookToggle.MouseButton1Click:Connect(function()
    ...
    webhookToggle.Text = webhookEnabled and "âœ… WEBHOOK ACTIVADO" or "âŒ WEBHOOK DESACTIVADO"
...
scanBtn.MouseButton1Click:Connect(function()
    ...
    if #found > 0 then
        showNotification(string.format("âœ… %d Brainrot(s)\nðŸ’¬ Enviado", #found), Color3.fromRGB(50, 200, 100))
    else
        showNotification("ðŸ” No hay Brainrots de 10M/s+", Color3.fromRGB(255, 150, 50))
    end
...
print("âœ… Hopper PRO cargado!")
print("ðŸ” Escaneo automÃ¡tico al entrar: ACTIVADO")
print("ðŸ’¬ Link clickeable en Discord: ACTIVADO")
print("ðŸ”„ Anti-rejoin: ACTIVADO")
