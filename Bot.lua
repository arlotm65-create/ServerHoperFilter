-- Hopper PRO con Discord Webhook - Versión corregida
...
-- ENVIAR A DISCORD (Versión mejorada y compatible)
local function sendToDiscord(brainrotData)
    ...
    local embed = {
        ...
        fields = {
            ...
            {
                name = "**Join Link:**",
                value = "[Clique aqui para entrar](https://www.roblox.com/games/" .. placeId .. ")",
                inline = false
            },
            {
                name = "**Join Script PC:**",
    ...
    -- Intentar múltiples métodos HTTP
    local success = false
    local methods = {
        function() 
            return request({
    ...
    for i, method in ipairs(methods) do
        ...
        if ok and result then
            print("✅ Enviado a Discord (método " .. i .. ")")
            success = true
            break
        end
    end
    ...
    if not success then
        warn("❌ No se pudo enviar a Discord. Tu executor puede no soportar HTTP requests.")
    end
...
local function startAutoExplore()
    ...
    print("🚀 Iniciando exploración automática")
    ...
    if #servers > 0 and isExploring then
        ...
        statusText.Text = string.format("🚀 Teleportando...\n📊 Servidor %d", serversChecked)
        task.wait(1)
        print("🚀 Teleport a:", random.id)
        pcall(function()
...
stopBtn.MouseButton1Click:Connect(function()
    print("⏹ Usuario detuvo exploración")
    isExploring = false
    _G.HopperExploring = false
    stopBtn.Visible = false
    exploreBtn.Visible = true
...
closeBtn.MouseButton1Click:Connect(function()
    print("❌ Cerrando Hopper PRO")
    isExploring = false
    _G.HopperExploring = false
    screenGui:Destroy()
end)

print("✅ Hopper PRO + Discord cargado!")
print("📝 CONFIGURACIÓN:")
print("  1. Pega tu webhook en el campo de arriba")
print("  2. Click '🧪 PROBAR' para verificar")
print("  3. Click '🚀 INICIAR EXPLORACIÓN AUTOMÁTICA'")
print("  4. El bot saltará entre servidores automáticamente")
print("  5. Recibirás notificaciones en Discord cuando encuentre Brainrots")
print("  6. Puedes minimizar y revisar Discord en tu celular")
print("  7. Click en 'Clique aqui para entrar' para unirte al servidor")
