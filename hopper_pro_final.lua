-- Hopper Script PRO (Final) - Server-by-server scanner y GUI
-- Escanea servidor por servidor (teleportando) hasta encontrar Brainrots que cumplan el mínimo seleccionado.
-- Modo de uso:
-- 1) Selecciona el mínimo en "Filtro Mínimo"
-- 2) Presiona "INICIAR EXPLORACIÓN" -> el bot irá servidor por servidor (teleportando) y buscará Brainrots
-- 3) Si encuentra Brainrots en el servidor actual, los listará y te notificará. Usa "ENTRAR" para unirte (teleport)
-- NOTA IMPORTANTE: Para poder escanear otros servidores, el script necesita teleportar al servidor objetivo. El script continúa ejecutándose tras el teleport (usa _G.HopperExploring).
-- Limitaciones técnicas: La lista local de resultados NO persiste entre servidores (esto es una limitación de Roblox). El script está diseñado para detenerse cuando encuentra un Brainrot y dejarte en ese servidor.
...
local function addResultToUI(entry)
    ...
    joinBtn.MouseButton1Click:Connect(function()
        ...
        if not success then
            ...
            -- notify failure
            local f = Instance.new("Frame"); f.Size=UDim2.new(0,360,0,72); f.Position=UDim2.new(1,-380,0,24); f.BackgroundColor3=Color3.fromRGB(220,50,50); f.Parent = screenGui; addCorner(f,10)
            local t = Instance.new("TextLabel"); t.Size=UDim2.new(1,-20,1,-20); t.Position=UDim2.new(0,10,0,10); t.BackgroundTransparency=1; t.Font=Enum.Font.GothamBold; t.TextSize=14; t.TextColor3=Color3.new(1,1,1); t.Text="❌ Falló el teleport a ese servidor"; t.Parent=f
...
local function startServerByServerExplore()
    ...
    if not servers or #servers == 0 then statusText.Text = "❌ No se obtuvieron servidores públicos."; _G.HopperExploring = false; return end
    ...
    if #localFound > 0 then
        for _,f in ipairs(localFound) do table.insert(foundServers, f) end
        statusText.Text = "🎉 Encontrado en este servidor! Revisar lista abajo."
        for _,f in ipairs(foundServers) do addResultToUI(f) end
    ...
    for idx,s in ipairs(order) do
        ...
        -- We use _G.HopperExploring to indicate we are mid-exploration; on post-teleport logic (below) we scan and if found we stop.
        return
    end
    -- if loop finished and no teleport succeeded
    _G.HopperExploring = false
    statusText.Text = "🔍 Exploración finalizada (no se encontraron brainrots en los servers revisados o no pudo teleportar)."
end
...
if _G.HopperExploring then
    ...
    if found and #found > 0 then
        for _,f in ipairs(found) do table.insert(foundServers, f) end
        ...
        -- stop exploring so you can decide to stay/join manually
        _G.HopperExploring = false
    else
        -- not found, continue exploring automatically: fetch server list and teleport to next
        task.wait(0.8)
        -- we will call startServerByServerExplore to continue (it will fetch list and teleport)
        -- but to avoid infinite loops, we stop here and let user re-press Explore if desired
